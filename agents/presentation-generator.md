...
1. **Preferred**: Call `plur_inject_hybrid` MCP tool with `prompt` = your task description and `scope` = `agent:presentation-generator`
2. **Fallback**: If MCP is unavailable, read `.datacore/state/agent-engrams/presentation-generator.md` for compiled engrams
